      ...
      >
        <div className="max-w-[1920px] mx-auto px-6 md:px-12 lg:px-20 flex justify-between items-center bg-transparent">
          ...
          <nav className="hidden lg:flex items-center gap-12 xl:gap-16">
            <Link href="/about" className={` hover:text-[#505153] text-lg font-thin tracking-wide transition-colors duration-300 ${isScrolled ? 'text-[#505153]' : 'text-[#505153]'}`}>About</Link>

            {/* Desktop Dropdown */}
            <div className="group relative py-4">
              <button className={`flex items-center gap-1  group-hover:text-[#505153] text-lg font-thin tracking-wide transition-colors duration-300 ${isScrolled ? 'text-[#505153]' : 'text-[#505153]'}`}>
                Projects <ChevronDown size={14} className="group-hover:rotate-180 transition-transform duration-300 text-gray-400 group-hover:text-[#505153]" />
              </button>
                    ...
                    <div className="flex flex-col gap-3">
                      {PROJECTS.ongoing.map(p => (
                        <Link key={p.name} href={p.href} className="group/item flex items-center justify-between transition-colors duration-300">
                          <span className="text-lg text-[#505153] group-hover/item:text-[#505153] transition-colors whitespace-nowrap font-medium">{p.name}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 border-l border-gray-100 pl-12">
                    ...
                    <div className="grid grid-cols-3 gap-x-8 gap-y-4">
                      {PROJECTS.completed.map(p => (
                        <Link key={p.name} href={p.href} className="block text-[15px] text-gray-600 hover:text-[#505153] transition-colors hover:translate-x-1 duration-200">{p.name}</Link>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <Link href="/project-enquire?project=rudraksh" className={`hover:text-[#505153] text-lg font-thin tracking-wide transition-colors duration-300 ${isScrolled ? 'text-[#505153]' : 'text-[#505153]'}`}>Contact</Link>
          </nav>
